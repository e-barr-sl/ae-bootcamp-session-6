...
1. **Given** a todo with a due date in the past and status incomplete, **When** the user views the todo list, **Then** the todo is visually marked as overdue (e.g., distinct color, icon, or label).
...
1. **Given** an overdue todo, **When** the user opens it to view or edit details, **Then** the overdue state is visible in the detail or form view.
...
- What happens when a todo with no due date has a due date added that is in the past?
...
- **FR-004**: The overdue indicator MUST be visually distinct from the normal todo appearance (e.g., different color, badge, or icon) so it is perceivable without relying solely on color.
...
- **SC-004**: The overdue indicator is perceivable by users who cannot distinguish colors alone (meets basic accessibility requirements).
